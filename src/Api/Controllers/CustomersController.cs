...
using Application.Customers.ImportCustomersData;
...
public class CustomersController(ISender mediator) : ApiControllerBase(mediator)
{
    ...
    {
        var result = await Mediator.Send(new ImportCustomersDataCommand(clientId, file));
    ...
    {
        var result = await Mediator.Send(new GetCustomerListQuery(clientId));
